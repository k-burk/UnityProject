...
public class Player : MonoBehaviour
{
    ...
    [Header("Gravity")]
    [Tooltip("Maximum downward momentum the player can have due gravity.")]
    ...
    [Tooltip("Multiplier for additional velocity gain when moving against " +
        "ongoing momentum.For example, 0 means no additional velocity, .5 means 50 % extra, etc.")]
    public float reverseMomentumMulitplier = .6f;
    [Tooltip("Multiplier for velocity influence when moving " +
        "midair.For example, .5 means 50 % speed.A value greater than 1 will make you move faster while midair.")]
    public float midairMovementMultiplier = .4f;
    [Tooltip("Multiplier for how much velocity is retained after bouncing" +
        "off of a wall.For example, 1 is full velocity, .2 is 20 %.")]
    [Range(0, 1)]
...
if (grounded)
        ...
        if (yVelocity > 0 && charController.collisionFlags.
        HasFlag(CollisionFlags.Above))
            yVelocity = 0;
    }
