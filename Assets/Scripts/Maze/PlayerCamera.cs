...
public class PlayerCamera1 : MonoBehaviour
{
    ...
    [Tooltip("The third-person camera will be kept this many units off " +
        "walls it touches.Setting this higher can help prevent the camera from clipping with bumpy walls.")]
    public float wallMargin = .5f;
    [Tooltip("The amount of smoothing applied to the first-person" +
        " camera.A higher value will cause the camera to more gradually turn when the mouse is moved.")]
    [Range(0, .99f)]
    ...
    [Tooltip("Multiplier for scroll wheel movement. A higher value will" +
        "result in a greater change in third - person distance when scrolling the mouse wheel.")]
    ...
    [Header("Misc")]
    [Tooltip("The layer mask for what the third-person camera will be obstructed by, and what " +
        "it will ignore and pass through.You'll probably want this to include environmental objects," +
        " but not smaller entities.")]
    ...
    [Tooltip("Is the camera currently in first-person mode (true) or third-person mode(false) ? " +
        "This can be set to determine the default mode when the game starts.")]
